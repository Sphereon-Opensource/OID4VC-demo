...
import {
    SSICredentialIssuerConnectPageConfig,
...
const SSICredentialIssuerConnectPage: React.FC = () => {
    const location = useLocation()
    const ecosystem = useEcosystem()
    const flowRouter = useFlowRouter<SSICredentialIssuerConnectPageConfig>()
    ...
    const createURI = (): string => {
        let uri = `openid-connect-issuer://?issuer_url=${encodeURIComponent(pageConfig.issuerUrl)}&client_id=${encodeURIComponent(pageConfig.clientId)}&redirect_uri=${encodeURIComponent(pageConfig.redirectUri)}`
        if (pageConfig.proxyTokenUrl) {
            uri += `&proxy_token_url=${encodeURIComponent(pageConfig.proxyTokenUrl)}`;
        }
        return uri
    }
...
export default SSICredentialIssuerConnectPage
